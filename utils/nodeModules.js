/**
 * This node_modules cannot depend on any third part libraries
 */
...
function addNodeModuleFromConfigJSON(){
    try{
        ...
        if(additionalNodeModules instanceof Array){
            pathArr = additionalNodeModules;
        }else if(typeof additionalNodeModules == 'string'){
            pathArr.push(additionalNodeModules);
        }
        for(let i=0; i<pathArr.length; i++){
            addPath(pathArr[i]);
        }
    }catch(err){
        console.error("addNodeModuleFromConfigJSON error: ",err);
    }
}
...
exports.addNodeModuleFromConfigJSON = addNodeModuleFromConfigJSON;